...
public class Player3 {
    ...
    public static final int p_step = 5;                 // 在没有赢或者输的情况,好的位置
    ...
    static class ProiortyAction implements Comparable<ProiortyAction> {
        ...
        public void downPriority(PriorityQueue queue, int priority){
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (isLevelOne){
                doLevelOne(opponentAction, actions, queue1, grid1, 0);
            }else{
    ...
    private static void doLevelTwo(ProiortyAction opponentAction, List<ProiortyAction> actions,
        ...
        doLevelOne(new ProiortyAction(), actions, queue2.get(action.ninePos), grid2.get(action.ninePos), action.ninePos);
    }
    ...
    private static void checkRowAndCol(int[][] grid, PriorityQueue<ProiortyAction> queue, int position) {
        for (int x = 0; x < 3; x ++){
            ...
            if (rowOpponentCount == 1 && rowAction.isValid()){
                rowAction.downPriority(queue, p_step);
                return;
            }
            if (colOpponentCount == 1 && colAction.isValid()){
                colAction.downPriority(queue, p_step);
                return;
            }
        }
    }
